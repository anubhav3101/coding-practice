Q- Given a non-empty array nums[] of integers of length N, find the top k elements which have the highest frequency in the array. If two numbers have same frequencies, then the larger number should be given more preference.
...
class Solution:
    def topK(self, a, k):
        ...
        sort = sorted(d.items(),key = lambda x : (x[1],x[0]),reverse = True)
        res = []
        for i in range(k):
            res.append(sort[i][0])
        return res
